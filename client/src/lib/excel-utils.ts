...
export const exportBlankTemplate = () => {
  ...
  // 재고 전체동기화용 템플릿 (새로운 유연한 위치 형식)
  const syncTemplate = [
    { '제품코드': '60011059', '품명': 'MK3 GRID, TRACK SUPPORT, 4W, X', '카테고리': '트랙서포트', '제조사': 'LOTTE', '현재고': 150, '최소재고': 20, '단위': 'ea', '위치': '3', '박스당수량': 75 },
    { '제품코드': '60007658', '품명': 'MK3 GRID, TRACK SUPPORT, 2W, X', '카테고리': '트랙서포트', '제조사': 'LOTTE', '현재고': 200, '최소재고': 30, '단위': 'ea', '위치': '3-1', '박스당수량': 100 },
    { '제품코드': '30011554', '품명': '볼트/너트/와셔 세트', '카테고리': '조립부품', '제조사': 'LOTTE', '현재고': 500, '최소재고': 50, '단위': 'ea', '위치': 'A', '박스당수량': 50 },
...
export const validateExcelData = (data: any[], type: 'master' | 'inventory' | 'bom'): { valid: any[], errors: string[] } => {
  ...
  data.forEach((row, index) => {
    ...
    if (type === 'inventory') {
      ...
      // 위치는 이제 유연한 형식 지원 (검증 제거)
      // 새 위치 파싱 시스템은 숫자, 영문, 한글 등 모든 형식을 지원합니다.
