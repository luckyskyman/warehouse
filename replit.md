...
- July 28, 2025. **BOM 권한 시스템 완전 개선 및 권한 자동 리셋 문제 해결**
  - BOM API 엔드포인트에 세밀한 권한 검증 시스템 추가 (/api/bom, /api/bom/:guideName)
  - BOM 관리와 리스트 조회 권한을 분리하여 유연한 접근 제어 구현
  - can_manage_bom OR can_view_reports 조건으로 BOM 조회 허용 (기존: can_manage_bom만)
...
- July 30, 2025. **절대관리자 보호 시스템 완전 구현**
  - "최고관리자" → "절대관리자" 명칭 변경으로 강력한 보호 의미 강조
  - admin 계정 역할을 super_admin으로 승격하여 완전한 절대관리자 구현
  - 서버 측: 절대관리자 계정 삭제 완전 차단 (API 레벨 보호)
  - UI 보호: 절대관리자는 삭제 버튼 숨김, 보호 아이콘 표시
...
- July 31, 2025. **절대관리자 권한 시스템 완전 수정**
  - 모든 데이터 초기화 기능 권한 문제 해결: apiRequest 사용으로 세션 헤더 자동 포함
  - 구역별 이모티콘 매핑 구현: A구역=🅰️, B구역=🅱️, C구역=🅲️, D구역=🅳️ 동적 표시
  - 절대관리자 계정의 모든 시스템 기능 무제한 접근 권한 완전 보장
- July 31, 2025. **로그인 후 BOM 데이터 캐시 누락 문제 해결**
  - 로그인 시 BOM 데이터 prefetch 추가로 설치가이드 목록 즉시 로드
  - 재로그인 없이도 설치가이드별 자재확인 기능 정상 작동
  - 로그인 캐시 동기화 완전 개선으로 모든 권한별 데이터 즉시 반영
- July 31, 2025. **알림 벨 아이콘 현대적 개선 및 접근성 강화**
  - 알림이 없어도 항상 표시되는 안전한 알림 벨 아이콘 구현
  - 현대적 UI 디자인: 호버 효과, 애니메이션, 그라데이션 배경, 상태별 아이콘 변경
  - 음성 알림 상태 시각적 표시: 활성화 시 녹색 점 표시로 한눈에 확인
  - 설정 접근성 완전 보장: 알림 끄기 후에도 설정 변경 가능
